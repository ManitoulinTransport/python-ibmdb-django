...
class DatabaseOperations ( BaseDatabaseOperations ):
    ...
    def date_trunc_sql( self, lookup_type, sql, params, tzname=None ):
        ...
        if lookup_type.upper() == 'DAY':
            return f" TIMESTAMP(DATE(SUBSTR(CHAR({sql}), 1, 10) || ''), TIME('00:00:00')) ", params
        elif lookup_type.upper() == 'MONTH':
            return f" TIMESTAMP(DATE(SUBSTR(CHAR({sql}), 1, 7) || '-01'), TIME('00:00:00')) ", params
        elif lookup_type.upper() == 'YEAR':
            return f" TIMESTAMP(DATE(SUBSTR(CHAR({sql}), 1, 4) || '-01-01'), TIME('00:00:00')) ", params
        return sql, params
